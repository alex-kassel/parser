...
class ProductListParser extends HtmlParser
{
    protected function processOutput(): void
    {
        $this->output = [
            ...
            'products' => $this->crawler->filter('.product--box')->each(function ($node) {
                return [
                    'title' => $node->filter('.product--title')->text(),
                    'description' => $node->filter('.product--description')->textRows(),
                    'image' => explode(' ', $node->filter('img')->attr('srcset'))[0],
                    'url' => $node->filter('a')->attr('href'),
                    'price-tax' => $node->filter('.product--price-info')->textRows(),
